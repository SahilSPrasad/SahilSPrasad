# Hi, I’m Sahil 👋

🎓 MS CS @ Georgia Tech (May 2028) · BS CS @ CSU Sacramento  
💻 Software Engineer — Backend & Distributed Systems  
📍 Sacramento, CA, United States  

---

## 🚀 About
I’m a software engineer with a strong interest in **backend systems, distributed architectures, and scalable services**. I enjoy building reliable systems using **Java, Spring Boot, and event-driven architectures**, and I’m always looking to deepen my understanding of system design and performance.

---

## 🛠 Tech
**Languages:** Java, Python, C++, SQL  
**Backend:** Spring Boot, REST, Kafka, JPA/Hibernate, React  
**Infra:** Docker, Terraform, AWS (EC2, S3), Git, Linux  
**Testing:** JUnit, Mockito  

---

## 💼 Experience
**Software Engineer Intern — Adaptive Biotechnologies** *(Seattle, WA | 2023)*  
**Software Engineer Intern — Pacific Coast Companies Inc** *(Rancho Cordova, CA | 2022–2023)*  

---

## 📌 Projects
**Topography Pathfinder (C++)** — Graph-based pathfinding with terrain-aware cost modeling  
**Farmfolio (React, Spring Boot, H2)** — Full-stack app with REST APIs and relational data models  
**Mandelbrot Visualizer (C++)** — High-performance fractal rendering via complex iteration  
**Rainmaker Engine (JavaFX)** — OOP game engine using state patterns and update loops  

---

## 📫 Connect
[LinkedIn](https://www.linkedin.com/in/sahilsprasad/) · sahilsprasad1@gmail.com
